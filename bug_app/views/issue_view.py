...
from bug_app.models import Issues, IssuesReply, ProjectUser
    ...
    if request.method=='POST':
        ...
        if response_dict['name'] in ['issues_type', 'module', 'parent', 'assign']:
            if response_dict['name'] == 'assign':
                ...
                # 条件判断：用户输入的值，是自己的值。
                response_dict['value'] = field.related_model.objects.filter(id=response_dict['value'], project_id=project_id).first()
